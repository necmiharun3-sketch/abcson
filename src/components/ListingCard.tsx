...
const ListingCard = ({ title, category, seller, price, oldPrice, imageColor, emoji = "🎮", views = Math.floor(Math.random() * 500 + 50), verified = Math.random() > 0.3, online = Math.random() > 0.4, to = '/ilan-pazari' }: ListingCardProps) => {
  const discount = oldPrice ? Math.round((1 - parseFloat(price.replace(/[^\d,]/g, '').replace(',', '.')) / parseFloat(oldPrice.replace(/[^\d,]/g, '').replace(',', '.'))) * 100) : 0;

  return (
    <Link to={to} className="group block bg-card rounded-xl border border-border overflow-hidden hover:border-primary/40 transition-all hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-0.5 duration-200">
      {/* Image area */}
