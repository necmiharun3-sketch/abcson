...
export default function AdminFinance() {
  ...
  const handleTopupRequest = async (t: any, nextStatus: 'completed' | 'failed') => {
    try {
      await runTransaction(db, async tx => {
        const txRef = doc(db, 'transactions', t.id);
        const currentSnap = await tx.get(txRef);
        if (!currentSnap.exists()) throw new Error('İşlem bulunamadı');
        const current = currentSnap.data() as any;
        if (current.status !== 'pending') throw new Error('Bu talep zaten işlenmiş');
        ...
        if (nextStatus === 'completed') {
          const userRef = doc(db, 'users', String(current.userId || ''));
          ...
          const next = curr + Math.round(Number(current.amount || 0) * 100);
          tx.update(userRef, {
            balanceAvailableCents: next,
            balance: next / 100,
            updatedAt: serverTimestamp(),
          });
        }
      });
      await logAction(`finance.topup.${nextStatus}`, t.id, { amount: Number(t.amount || 0), method: t.method || '' });
      toast.success(nextStatus === 'completed' ? 'Bakiye yükleme onaylandı.' : 'Bakiye yükleme reddedildi.');
      load();
    } catch (e: any) {
      toast.error(e?.message || 'İşlem güncellenemedi.');
    }
  };
              ...
              <TableBody>
                {filteredTx.slice(0, 100).map(t => (
                  <TableRow key={t.id} className="border-white/10 hover:bg-white/5">
                    ...
                    <TableCell className="text-right">
                      {t.type === 'topup' && t.status === 'pending' && isStaff ? (
                        <div className="flex gap-1 justify-end">
                          <Button size="sm" className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 h-7 px-2" onClick={() => handleTopupRequest(t, 'completed')}>
                            <CheckCircle className="w-3 h-3 mr-1" />Onayla
                          </Button>
                          <Button size="sm" variant="outline" className="border-red-500/20 text-red-400 h-7 px-2" onClick={() => handleTopupRequest(t, 'failed')}>
