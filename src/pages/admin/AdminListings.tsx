...
export default function AdminListings() {
  ...
  const moderate = async (p: any, moderationStatus: string, reason = '') => {
    try {
      await updateDoc(doc(db, 'products', p.id), {
        ...
        status: moderationStatus === 'approved' ? 'active' : ['rejected','suspended'].includes(moderationStatus) ? 'inactive' : p.status,
  ...
  const featureListing = async (p: any) => {
    ...
    await updateDoc(doc(db, 'products', p.id), { isFeatured: !isFeatured, isVitrin: !isFeatured, updatedAt: serverTimestamp() });
    await logAction('product.feature', p.id, { isFeatured: !isFeatured });
    toast.success(isFeatured ? 'Öne çıkarma kaldırıldı.' : 'İlan öne çıkarıldı.');
    setProducts(prev => prev.map(x => x.id === p.id ? { ...x, isFeatured: !isFeatured, isVitrin: !isFeatured } : x));
  };
  ...
  const filtered = useMemo(() => {
    let r = products;
    if (search) r = r.filter(p => (p.title || '').toLowerCase().includes(search.toLowerCase()) || String(p.sellerId || p.userId || '').includes(search));
  ...
  const reported = products.filter(p => (p.reportCount || 0) > 0 || !!p.flagged);
          ...
          <TableRow key={p.id} className="border-white/10 hover:bg-white/5">
            ...
            <TableCell>
              <div className="flex items-center gap-2.5">
                {(p.imageUrls?.[0] || p.image) ? <img src={p.imageUrls?.[0] || p.image} className="w-10 h-10 rounded object-cover" alt="" /> : <div className="w-10 h-10 rounded bg-white/5 flex items-center justify-center"><Package className="w-4 h-4 text-gray-600" /></div>}
                <div className="max-w-[180px]">
                  <p className="text-white text-sm font-medium truncate">{p.title || 'İsimsiz'}</p>
                  {!!(p.isFeatured || p.featured || p.isVitrin) && <span className="text-xs text-purple-400">⭐ Öne Çıkan</span>}
                  {(p.reportCount > 0 || p.flagged) && <span className="text-xs text-red-400 ml-1">⚠ {p.reportCount || 1} rapor</span>}
                </div>
              </div>
            </TableCell>
            <TableCell className="text-gray-400 text-sm">{p.sellerName || p.username || p.sellerId?.slice(0, 8) || p.userId?.slice(0, 8) || '-'}</TableCell>
            ...
            <TableCell className="text-right">
              <div className="flex gap-1 justify-end">
                ...
                <Button size="sm" variant="outline" className={`h-7 px-2 ${(p.isFeatured || p.featured || p.isVitrin) ? 'border-purple-500/30 text-purple-400' : 'border-white/10 text-gray-500'}`} onClick={() => featureListing(p)}><Star className="w-3 h-3" /></Button>
