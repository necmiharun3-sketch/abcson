...
export default function IlanEkle() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const requireStrictListingVerification =
      import.meta.env.VITE_REQUIRE_STRICT_LISTING_VERIFICATION === "true";
    ...
    if (requireStrictListingVerification && !profile?.smsVerified) {
      toast.error("İlan eklemek için telefon doğrulaması zorunludur.");
      return;
    }

    if (requireStrictListingVerification && profile?.kycStatus !== "verified") {
      toast.error("İlan eklemek için KYC doğrulaması zorunludur.");
      return;
    }

    if (!requireStrictListingVerification && profile?.kycStatus === "rejected") {
      toast.error("KYC durumunuz reddedildiği için ilan ekleyemezsiniz. Lütfen yeniden doğrulama başlatın.");
      return;
    }
